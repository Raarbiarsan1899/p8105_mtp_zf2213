...
time series observations. We can visually feel the daily total activity
are increasing over time. For statistical analysis, it would be better
removing NAs.
...
least during Tuesdays, according to the boxplot in Figure.2.

The increases of active during Friday, Saturday and Sunday over time are
more significant that that of other days in the week. But the trends are
the same for each day in the week.
...
``` r
activity %>%
    ggplot(aes(x = counts)) + scale_x_log10(breaks = c(2, 4, 8, 16, 32)) + 
    geom_density()
```

<img src="p8105_mtp_zf2213_files/figure-gfm/heatmap-2.png" width="100%" />

The patient are active during days and sedatory during night. He seems
to live a regular life, and slightly more active during night than days.

Regarding the data, there seem to be some missing days (as mentioned in
Figure.1), and some “strange” days with extremely low variance. Refer to
...
To summarize, the accelerometer data provides observations of daily
activity of a patient. Wrangling more sophisticated information from
this dataset requires more advanced statistical learning knowledge and
programming skills.
