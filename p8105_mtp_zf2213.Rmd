...
activity <- activity %>% 
	...
	mutate(hour_in_day = minute_in_day %/% 60, minute_in_hour = minute_in_day %% 60) %>% 
	...
	# range the variable
	select(day, week, day_in_week, hour_in_day, minute_in_hour, minute_in_day, counts)
...
# percentage of time remains sedentary 
acc %>%
	group_by(day_of_period, day, range_accel) %>%
	summarize(sum_activity = n()) %>%
	spread(key = range_accel, value = sum_activity, fill = 0) %>% 
	mutate(prop_active = active / (sed + active)) %>%
	ggplot(aes(x = day_of_period, y = prop_active)) +
	  geom_line() +
	  geom_smooth() +
	  facet_grid(. ~ day)
...
(reate a total activity variable)
...
acc %>%
	ggplot(aes(x = time_activity, y = day_of_period, color = log10(accel))) +
